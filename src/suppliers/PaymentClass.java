...
try{
        ...
        boolean isAdded = db.go.runNonQuery(Insert);
                if(isAdded){
                    TheTools.msgBox("تم اضافة عملية الدفع");
                }
        }catch(Exception ex){
            TheTools.msgBox(ex.getMessage());
              TheTools.msgBox("لم يتم اضافة عملية الدفع");
        }
    }
     public void addinstore() {
       ...
       public void addinsupbalance() {
        try {
            ...
            if (isAdded) {
                TheTools.msgBox("تم خصم  الدفعة من الرصيد ");
            }
        } catch (Exception ex) {

            TheTools.msgBox(ex.getMessage());
            TheTools.msgBox("لم يتم خصم  الدفعة من الرصيد ");
        }
